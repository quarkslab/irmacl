...
class TestCornerCase(unittest.TestCase):
    ...
    def test_utf8(self):
        force = False
        scan = scan_files(UTF8_PATHS, force, blocking=True, session=SESSION)
        for get_result in scan.results:
            res = scan_proberesults(get_result.id, session=SESSION)
            self.assertIn(res.name, UTF8_SAMPLES)
        for filename in UTF8_SAMPLES:
            (_, res) = file_search(filename, limit=1, session=SESSION)
